    ...
    async def apply(self, url: str, profile: ApplicantProfile) -> ApplyResult:
        try:
            ...
            task_prompt = (
                ...
                f"- To fill a text field: look at the interactive element indices in the screenshot, "
                f"then call input(index=<NUMBER>, text='value', clear=true). "
                f"Do NOT use find_elements to discover indices — it does NOT return interactive indices. "
                f"Instead, read the index numbers shown next to elements in the screenshot.\n"
                ...
                f"built-in 'input' action with 'clear: true'. This types character by character and "
                f"works with React/Workday. Do NOT use set_form_value for visible text inputs — "
                f"it sets the DOM value but React overwrites it on re-render.\n"
                ...
                f"If a field doesn't work, try a different method or SKIP it.\n"
                f"- If ALL methods fail for a field after 3 total attempts, SKIP it and move on.\n"
                f"- NEVER call find_elements more than once for the same field. "
                f"If you need an element's index, look at the screenshot or click near the field first.\n\n"

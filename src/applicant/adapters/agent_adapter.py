    ...
    async def apply(self, url: str, profile: ApplicantProfile) -> ApplyResult:
        try:
            ...
            task_prompt = (
                ...
                # ── browser-use upstream (AGENTS.md — Agent Prompting Guide) ──
                ...
                f"- Read the PREVIOUS step's ActionResult / tool output before deciding the next tool; "
                f"prefer structured hints from custom tools over guessing CSS.\n\n"
                ...
                f"  - Prefer force_click_element(text='exact visible option') for option rows; "
                f"if that fails, use click(index=N) on the option line from the CURRENT step's element list "
                f"(browser-use snapshot index — valid for visible rows).\n"
                f"  - Avoid unindexed generic 'click' without a number; for React lists, force_click or "
                f"indexed click are reliable.\n"
